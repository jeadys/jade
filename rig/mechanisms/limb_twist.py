...
class Twist:
    ...
    def twist_joint(self, parent_segment: Segment, start_segment: Segment, end_segment: Segment,
        ...
        joint_group = f"{self.prefix}{self.name}_{self.module_nr}_TWIST_GROUP"
    ...
    def setup_twist_hierarchy(self, start_joint, end_joint):
        mult_matrix = cmds.createNode("multMatrix", name=f"{self.prefix}{self.name}_{self.module_nr}_multMatrix_#")
        decompose_matrix = cmds.createNode("decomposeMatrix",
                                           name=f"{self.prefix}{self.name}_{self.module_nr}_decomposeMatrix_#")
        quat_to_euler = cmds.createNode("quatToEuler",
                                        name=f"{self.prefix}{self.name}_{self.module_nr}_quatToEuler_#")
        ...
        for x in range(twist_amount):
            ...
            between_joint = cmds.joint(name=f"{self.prefix}{self.name}_{self.module_nr}_TWIST_#",
            ...
            multiply_divide = cmds.createNode("multiplyDivide",
                                              name=f"{self.prefix}{self.name}_{self.module_nr}_multiplyDivide_#")
