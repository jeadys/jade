...
class LegSwitch:
    ...
    def create_leg_switch(self):
        ik_fk_switch_offset, ik_fk_switch_ctrl = self.create_ik_fk_switch_leg_control(control_name="leg")
    ...
    def create_ik_fk_switch_leg_constraints(self, control_name: str, ik_fk_switch_ctrl):
        ...
        cmds.connectAttr(f"{ik_fk_switch_ctrl}.IK_FK_SWITCH", f"{self.prefix}_leg_IK_CTRL.visibility")
        cmds.connectAttr(f"{ik_fk_switch_ctrl}.IK_FK_SWITCH", f"{self.prefix}_knee_IK_CTRL.visibility")
        ...
        cmds.connectAttr(f"{reverse_node}.outputX", f"{self.leg_segments[0]}_FK_CTRL.visibility")
    ...
    def create_ik_fk_switch_leg_attribute_proxies(self, ik_fk_switch_ctrl):
        ...
        cmds.addAttr(f"{self.prefix}_leg_IK_CTRL", longName="IK_FK_SWITCH", proxy=f"{ik_fk_switch_ctrl}.IK_FK_SWITCH")
        cmds.addAttr(f"{self.prefix}_knee_IK_CTRL", longName="IK_FK_SWITCH", proxy=f"{ik_fk_switch_ctrl}.IK_FK_SWITCH")
