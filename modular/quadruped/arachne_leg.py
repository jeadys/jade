...
class ArachneLeg:
    ...
    def __init__(self, node: str, segments: list[Segment], prefix: Literal["L_", "R_"] = ""):
        ...
        self.ik_chain: IKChain = IKChain(node=node, name=ArachneLeg.name, prefix=self.prefix)
        self.fk_chain: FKChain = FKChain(node=node, name=ArachneLeg.name, prefix=self.prefix)
        self.stretch: Stretch = Stretch(node=node, name=ArachneLeg.name, prefix=self.prefix)
        self.twist: Twist = Twist(node=node, name=ArachneLeg.name, prefix=self.prefix)
    ...
    def inverse_kinematic(self) -> None:
        ...
        self.ik_controls = self.ik_chain.ik_control(segments=self.segments)
    ...
    def generate_arachne_leg(self) -> None:
        self.base_skeleton()
