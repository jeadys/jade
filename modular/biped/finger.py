...
class Finger:
    ...
    def __init__(self, node, segments: list[Segment], prefix: Literal["L_", "R_"] = ""):
        ...
        self.ik_chain: IKChain = IKChain(node=self.node, name=self.name, prefix=self.prefix)
        self.fk_chain: FKChain = FKChain(node=self.node, name=self.name, prefix=self.prefix)
        self.stretch: Stretch = Stretch(node=self.node, name=self.name, prefix=self.prefix)
