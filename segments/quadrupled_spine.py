...
class QuadrupledSpine:
    name: str = "quadrupled_spine"
    ...
    def __init__(self, prefix, spine_count: int):
        ...
        self.segments: list[str] = [f"{self.prefix}_quadrupled_spine_0{spine + 1}" for spine in range(self.spine_count)]
        self.positions: list[tuple] = [(0, 50, -35 + (self.distance_between_spines * spine)) for spine in
    ...
    def create_spine_controls(self, rotation_order: str):
        fk_instance = FKChain(prefix=self.prefix, name=QuadrupledSpine.name)
        ...
        spine_ik_instance = SplineChain(prefix="C", name=QuadrupledSpine.name)
        ...
        switch_instance = FKIKSwitch(prefix=self.prefix, fk_joints=fk_joints, ik_joints=ik_joints, fk_ctrls=fk_ctrls,
                                     ik_ctrls=ik_ctrls, name=QuadrupledSpine.name)
