...
class ArmTwist:
    ...
    def create_twist_joints(self, base_joint, end_joint):
        twist_a = cmds.duplicate(base_joint, parentOnly=True, name=f"{self.prefix}_arm_twist_a01")[0]
        cmds.setAttr(f"{twist_a}.radius", 5)
        twist_d = cmds.duplicate(end_joint, parentOnly=True, name=f"{self.prefix}_arm_twist_d01")[0]
    ...
    def create_twist_joints_hierarchy(self, twist_d, distance_between_value):
        twist_b = cmds.duplicate(twist_d, parentOnly=True, name=f"{self.prefix}_arm_twist_b01")[0]
        twist_c = cmds.duplicate(twist_d, parentOnly=True, name=f"{self.prefix}_arm_twist_c01")[0]
    ...
    def setup_upper_arm(self):
        twist_a, twist_d = self.create_twist_joints(self.arm_segments[0], self.arm_segments[1])
        ...
        cmds.pointConstraint(self.arm_segments[1], ik_handle_shoulder, maintainOffset=False)
        cmds.orientConstraint(self.arm_segments[0], twist_d, maintainOffset=False)
    ...
    def setup_lower_arm(self):
        twist_a, twist_d = self.create_twist_joints(self.arm_segments[-1], self.arm_segments[1])
        distance_between_value, _ = calculate_distance_between(twist_a, twist_d)
        twist_b, twist_c = self.create_twist_joints_hierarchy(twist_d, distance_between_value)
        ik_handle_wrist = self.create_ik_handle(twist_a, twist_d)
        cmds.parent(ik_handle_wrist, self.arm_segments[-1])
        cmds.pointConstraint(self.arm_segments[1], ik_handle_wrist, maintainOffset=False)
        cmds.orientConstraint(self.arm_segments[1], twist_d, maintainOffset=False)
    ...
    def create_arm_twist(self):
        self.setup_upper_arm()
        self.setup_lower_arm()
